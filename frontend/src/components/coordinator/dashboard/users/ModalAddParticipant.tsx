...
export function ModalAddParticipant({ children }: PropsType) {
    // Params
    ...
    const { mutate, isPending, error } = useMutation({
        mutationKey: ["add-participant-details"],
        ...
        onSuccess: () => {
            ...
            notify("New participant added successfully 👏");
            reset();
        },
        onError: (error) => {
            console.log(error);
            ...
            <DialogContent className="w-full flex flex-col gap-10 bg-my-bg border border-zinc-800">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-muted-foreground font-medium text-xs text-start">
                        You can add new participant's detials here.
                    </DialogDescription>
                </DialogHeader>
